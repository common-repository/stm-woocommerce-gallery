...
/*
Plugin Name: STM WooCommerce Gallery v.1.9
Plugin URI: sergiotoca.com
Description: Plugin para mostrar composiciones de imágenes de productos de WooCommerce
Version: 1.9
...
///////////////////////////////////////////
// STM PILI FOR WOOCOMMERCE              //
// Fecha inicio: 18 / IX / 16            //
// Inicio esta compilación: 28 / XI / 16 //
// Finalización: 26 / XII / 16           //
///////////////////////////////////////////
...
4. Función de copiado de plantilla de página específica
...
function stmpili_funcion_crear_taxonomia() {
	...
	register_taxonomy( 'stmpili_txn', array( 'product' ), $args );
...
/////////////////////////////////////////////////////////////////////////////
//                                                                         //
// 4. FUNCION DE COPIADO DE PLANTILLA DE PAGINA ESPECIFICA                 //
//                                                                         //
/////////////////////////////////////////////////////////////////////////////

add_action( 'after_setup_theme', 'stmpili_copia_plantilla_tema' );
	
function stmpili_copia_plantilla_tema() {
		
	$plantilla = 'stmpili-template.php';
...
function stmpili_funcion_creacion_pila( $composicion ) {
	...
	if ( have_posts() ) {
		while ( have_posts() ) {
			...
			if ( has_post_thumbnail() ) {
				...
				$valoracion = ( $product->get_average_rating() == 'NaN' ) ? 0 : $product->get_average_rating();
				$fichero = wp_get_attachment_url( get_post_thumbnail_id() );
	...
	foreach ( $auxiliar as $pieza ) {
		...
		$stmpili_pila[$contador]['descripcion'] = $pieza['descripcion'];
		$stmpili_pila[$contador]['precio'] = $pieza['precio'];
		$stmpili_pila[$contador]['valoracion'] = $pieza['valoracion'];
		$stmpili_pila[$contador]['tipo'] = $pieza['tipo'];
		$id = $pieza['id'];
		if ( mt_rand( 1, 10 ) > 5 ) {
			$stmpili_pila[$contador]['destacada'] = 'si';
			$stmpili_pila[$contador]['ancho'] = $pieza['ancho'] * 2;
			$stmpili_pila[$contador]['alto'] = $pieza['alto'] * 2;
		} else {
			$stmpili_pila[$contador]['destacada'] = 'no';
...
function stmpili_funcion_estilos_css( $composicion, $ancho ) {
	...
	switch ( $proporcion ) {
		case 0.8:
			...
			break;
		case 0.6:
			$estilo .= '.titulo-enlace, .icon-cancel-circled { font-size: medium; }';
			break;
		case 0.4:
			$estilo .= '.titulo-enlace, .icon-cancel-circled { font-size: small; }';
			break;
	}
...
// FUNCION DE HIGIENIZACION Y VALIDACION DE ENTRADA DE DATOS
// Fecha creación: 29 / XII / 16
